...
namespace MetaModels\AttributeTextBundle\Test\Migration;
...
use MetaModels\AttributeTextBundle\Migration\AllowNullMigration;
use PHPUnit\Framework\TestCase;

/**
 * @covers \MetaModels\AttributeTextBundle\Migration\AllowNullMigration
 */
class AllowNullMigrationTest extends TestCase
{
    public function testName(): void
    {
        ...
        self::assertSame('Allow null values in MetaModels "text" attributes.', $migration->getName());
    }
    ...
    public function testRun(object $configuration): void
    {
        ...
        if ($configuration->requiredTablesExist && $configuration->attributeConfigured) {
            ...
            $table1Columns = [
                'normal' =>
                    (new Column('normal', new StringType()))->setDefault($columnDefault),
                'camelcase' =>
                    (new Column('camelCase', new StringType()))->setDefault($columnDefault)
            ];
            $table2Columns = [
                'normal' =>
                    (new Column('normal', new StringType()))->setDefault($columnDefault),
